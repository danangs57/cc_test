...
const post = props.post ?? { title: '', content: '', id: null };
...
onMounted(async () => {
  await nextTick();
  if (!quillInitialized.value && post.id) {
    try {
      editor.value = new Quill(editorContainer.value, {
        theme: 'snow',
        placeholder: 'Write your post content here...'
      });
      editor.value.root.innerHTML = form.content;
...
<template>
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 py-16 flex flex-col min-h-screen">
    <div class="max-w-2xl w-full mx-auto flex-1">
      <div v-if="!post.id" class="bg-red-100 text-red-700 p-6 rounded-xl text-center font-bold text-lg mb-10">
        Error: Post data is missing. Please return to the posts list.
      </div>
      <div v-else>
        <div class="mb-10 text-center">
          <h1 class="text-5xl font-extrabold text-gray-900 mb-2">Edit Post</h1>
          <p class="text-gray-500 text-lg">Update your blog post below</p>
        </div>
        <form @submit.prevent="form.put(`/posts/${post.id}`)" class="space-y-8 bg-white p-10 rounded-3xl shadow-2xl border border-gray-100">
          <div>
          ...
          <div class="flex gap-4 justify-end">
            <button type="submit" class="bg-blue-600 text-white px-8 py-3 rounded-lg font-bold shadow hover:bg-blue-700 transition text-lg">Update</button>
